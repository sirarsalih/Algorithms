...
void MultilevelClustering::cluster(std::ofstream &foutput, std::map<std::string, int> &mapClustersStates, int &bestSoFar, std::string numberOfXLiterals, int &levelCounter, std::map<std::string, std::string> &mapLevelClusters, std::vector<std::string> vectorStringNumbers, int numberOfLiterals, std::string numberOfClauses, std::vector<std::string> &initialClusterCollection, std::vector<std::string> &clusterCollection, std::map<std::string, bool> &mapLiteralValues, std::multimap<std::string, bool> &mapClauseValues, clock_t begin)
{
	...
	while(firstEntrance || clusterCollection.size() >= atoi(numberOfXLiterals.c_str()) * 0.10)
	{
		...
		//We have reached the final level, assign the states and TRUE/FALSE values
		if(clusterCollection.size() <= atoi(numberOfXLiterals.c_str()) * 0.10)
		{
			std::cout << "Assigning the states of clusters randomly....." << std::endl << std::endl;
			std::cout << "Assigning the clusters TRUE/FALSE values....." << std::endl << std::endl;
			...
			//Assign the clusters random state values as well as the literals within the clusters
