...
void TabuSearch::run(std::ofstream &foutput, int &bestSoFar, std::string noLettersCollectionString, int &numberOfLiterals, int levelCounter, std::map<std::string, std::string> mapLevelClusters, std::vector<std::string> vectorStringNumbers, std::vector<std::string> &initialClusterCollection, std::vector<std::string> clusterCollection, std::map<std::string, bool> &mapLiteralValues, std::multimap<std::string, bool> &mapClauseValues, std::string numberOfXLiterals, std::string numberOfClauses, std::multimap<std::string, std::vector<std::string>> &multiMapLiteralClause, clock_t begin)
{
	...
	while(current < NUMBER_OF_CLAUSES)
	{
		...
		//If we have arrived at the last level, then normal TS will run
		if(levelCounter == 0)
		{
			if(firstTime)
			{
				...
				//Update SAT and find unsatisfied clauses
